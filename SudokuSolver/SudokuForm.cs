...
namespace SudokuSolver
{
    public partial class SudokuForm : Form
    {
        ...
        //Creating sudoku grid and setting its size
        ...
        private void CreateCells()
        {
            for (int i = 0; i < 9; i++)
            {
                for (int j = 0; j < 9; j++)
                {
                    ...
                    // Assign key press event for each cells2
                    cells[i, j].KeyPress += Cell_keyPressed;
                    cells[i, j].KeyDown += DelCell_keyPressed;

                    //Adding cell instance to the panel
                    sudokuGridPanel.Controls.Add(cells[i, j]);
                }
            }
        }
        ...
        private void Cell_keyPressed(object sender, KeyPressEventArgs e)
        {
            ...
            {
                // Clear the cell value if pressed key is zero or pressed delete
                if (value == 0) cell.Clear();
                else cell.Text = value.ToString();

                cell.ForeColor = Color.Blue;
            }
        }

        private void DelCell_keyPressed(object sender, KeyEventArgs e)
        {
            var cell = sender as SudokuCell;
        ...
        private void ClearInputButton_Click(object sender, EventArgs e)
        {
            foreach (var cell in cells)
            {
                ...
                if (cell.IsLocked == false) cell.Clear();
            }
        }
        ...
        private void NewGameButton_Click(object sender, EventArgs e)
        {
            ...
            // level player chosen
            if (BeginnerLevel.Checked) hintsCount = 45;
            else if (IntermediateLevel.Checked) hintsCount = 30;
            else if (AdvancedLevel.Checked) hintsCount = 15;
        ...
        private void SolveButton_Click(object sender, EventArgs e)
        {
            foreach (var cell in cells)
            {
                ...
                if (cell.IsLocked == false) cell.ForeColor = Color.Green;
